...
# Cloud-URL bleibt wie von dir vorgegeben
...
def run_scrapper():
    ...
    for spieltag in missing_days:
        ...
        try:
            ...
            # Alle Links durchsuchen, die das Analyse-Muster oder Ergebnisse enthalten
            ...
            for link in all_links:
                ...
                # Wir suchen nach dem "Zahl:Zahl" Muster (Ergebnis) im Link-Text
                if re.search(r'\d+:\d+', text_content):
                    # Wir nehmen den Text des Eltern-Elements für den Kontext (Teams + Ergebnisse)
                    raw_line = link.parent.get_text(" ", strip=True)
                    
                    # Regex zum Trennen: Heim (1) | Ergebnis (2) | Halbzeit (3) | Gast (4)
